...
export default async function ProfilePage({
  ...
  return (
    <div
      // On small screens, this is a vertical layout; on md+ screens, a row
      className="flex flex-col md:flex-row items-start justify-center bg-transparent text-text pt-16 sm:pt-24"
    >
      ...
      <div className="w-full md:w-1/3 flex flex-col items-center justify-between">
        <div>
          <img
            ...
            // Make image smaller on mobile, larger on bigger screens
            className="w-40 h-40 sm:w-60 sm:h-60 rounded-lg object-cover shadow-lg"
          />
          <div className="bg-gray-600 p-3 sm:p-4 rounded-lg shadow-lg text-center mt-4 w-full">
            <h1 className="text-xl sm:text-2xl text-[#d9a14e] font-bold">
              {profile.username}
            </h1>
            <p className="text-lg sm:text-xl text-[#cba162] font-semibold mt-2">
              {profile.age} | {profile.gender} | {profile.year_of_study}
            </p>
            <p className="text-base sm:text-lg text-[#e5c99d] font-semibold">
              {profile.major}
            </p>
          </div>
        </div>
      ...
      <div className="w-full md:w-2/3 mt-6 md:mt-0 md:ml-6 flex-shrink-0">
        <Tabs defaultValue="profile">
          ...
          <TabsContent
            value="profile"
            // Let the container shrink on mobile; on sm+ screens max 650px
            className="p-4 max-w-full sm:max-w-[650px]"
          >
          ...
          {isOwnProfile && (
            <TabsContent
              value="likes"
              className="flex-grow overflow-auto max-h-90 p-4 max-w-full sm:max-w-[650px]"
            >
